...
// MARK: - RoutineCorrelationsCard

struct RoutineCorrelationsCard: View {
    let data: RoutineCorrelations

    private var hasEnoughData: Bool { data.dataPoints >= data.minPoints }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !hasEnoughData {
                ...
                if data.routineStreak >= 3 { streakBadge }
                corrRows
            }
        }
        .padding(16)
    ...
    private var header: some View {
        HStack(spacing: 6) {
            ...
            Text("ROUTINE DE SOIR")
                .font(.system(size: 11, weight: .bold))
                .tracking(2)
                .foregroundColor(Color(white: 0.45))
            Spacer()
            Text("\(data.dataPoints) jours")
                .font(.system(size: 10))
    ...
    private var notEnoughDataView: some View {
        let remaining = data.minPoints - data.dataPoints
        return HStack(spacing: 8) {
            Image(systemName: "clock")
                .font(.system(size: 12))
                .foregroundColor(Color(white: 0.3))
            Text("Encore \(remaining) soir\(remaining > 1 ? "s" : "") de données pour activer les corrélations.")
                .font(.system(size: 12))
    ...
    private var streakBadge: some View {
        HStack(spacing: 6) {
            Image(systemName: "flame.fill")
                .font(.system(size: 11))
                .foregroundColor(.orange)
            Text("\(data.routineStreak) soirs consécutifs complétés")
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(.orange)
            Spacer()
        }
    ...
    private var corrRows: some View {
        VStack(spacing: 2) {
            if let corr = data.corr1Hrv, let delta = data.delta1HrvMs {
                corrRow(icon: "heart.fill",    label: "Routine → HRV lendemain",      corr: corr, detail: deltaStr(delta, unit: "ms"))
            }
            if let corr = data.corr2Sleep, let delta = data.delta2SleepMin {
                corrRow(icon: "moon.zzz.fill", label: "Routine → Sommeil lendemain",  corr: corr, detail: deltaStr(delta, unit: "min"))
            }
            if let corr = data.corr3Readiness, let delta = data.delta3Readiness {
                corrRow(icon: "bolt.fill",     label: "Routine → Readiness lendemain", corr: corr, detail: deltaStr(delta, unit: "pts"))
            }
            if let corr = data.corr4BestItem, let name = data.corr4BestItemName {
                corrRow(icon: "star.fill",     label: "Meilleur item → Sommeil",      corr: corr, detail: itemLabel(name))
            }
            if let corr = data.corr5BedtimeHrv, let slope = data.slope5MsPerHour {
                corrRow(icon: "bed.double.fill", label: "Heure coucher → HRV",        corr: corr, detail: slopeStr(slope))
            }
        }
    }

    private func corrRow(icon: String, label: String, corr: Double, detail: String) -> some View {
        let barColor = corrColor(corr)
        let barFraction = CGFloat(min(abs(corr), 1.0))
        return HStack(spacing: 10) {
            Image(systemName: icon)
                .font(.system(size: 10))
                .foregroundColor(barColor.opacity(0.7))
                .frame(width: 14)
            VStack(alignment: .leading, spacing: 3) {
                ...
                GeometryReader { geo in
                    let w = geo.size.width * barFraction
                    ZStack(alignment: .leading) {
                        Capsule().fill(Color.white.opacity(0.06)).frame(height: 3)
                        Capsule().fill(barColor).frame(width: w, height: 3)
                    }
                }
                .frame(height: 3)
            }
    ...
    private func corrColor(_ r: Double) -> Color {
        if r >= 0.3  { return .green }
        if r <= -0.3 { return .red }
        return Color(white: 0.5)
    }

    private func deltaStr(_ v: Double, unit: String) -> String {
        let sign = v >= 0 ? "+" : ""
        return "\(sign)\(Int(v)) \(unit)"
    }

    private func slopeStr(_ slope: Double) -> String {
        let sign = slope >= 0 ? "+" : ""
        return "\(sign)\(String(format: "%.1f", slope)) ms/h"
    }

    private func itemLabel(_ key: String) -> String {
        switch key {
        case "routine_no_food":         return "Stop manger"
        case "routine_dim_lights":      return "Lumières"
        case "routine_shower":          return "Douche"
        case "routine_connection":      return "Connexion"
        case "routine_deconnect":       return "Déco écrans"
        case "routine_priorities_done": return "Priorités"
        case "routine_bedtime_ok":      return "Au lit"
        default:                        return key
        }
    }
}
