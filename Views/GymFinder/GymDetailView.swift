...
struct GymDetailView: View {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button { vm.toggleFavorite(gym) } label: {
                        Image(systemName: vm.isFavorite(gym) ? "star.fill" : "star")
                            .foregroundColor(vm.isFavorite(gym) ? Color.forge : Color.appOnSurface.opacity(0.5))
                    }
                }
            }
    ...
    private func vibeRow(label: String, value: Int, low: String, high: String) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            HStack {
                Text(low).font(.appMicro).foregroundColor(Color.appOnSurface.opacity(0.25))
                Spacer()
                Text(high).font(.appMicro).foregroundColor(Color.appOnSurface.opacity(0.25))
            }
...
struct AdaptWorkoutSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button("Fermer") { dismiss() }.foregroundColor(Color.appOnSurface.opacity(0.5))
                }
            }
        }
    }
    ...
    private var profilePicker: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("PROFIL D'ÉQUIPEMENT")
                .font(.appCaption.weight(.bold)).tracking(1.5).foregroundColor(Color.appOnSurface.opacity(0.35))
                ...
                HStack(spacing: 8) {
                    ForEach(EquipmentProfile.presets) { preset in
                        ...
                        Button { vm.selectedEquipmentProfile = selected ? nil : preset } label: {
                            ...
                            .foregroundColor(selected ? .black : Color.appOnSurface.opacity(0.65))
                    ...
                    if gym.crowdsource != nil {
                        ...
                        Button { vm.selectedEquipmentProfile = nil } label: {
                            ...
                            .foregroundColor(autoSelected ? .black : Color.appOnSurface.opacity(0.65))
                            .padding(.horizontal, 12).padding(.vertical, 9)
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ForEach(needed, id: \.rawValue) { eq in
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(eq.label)
                            .font(.appCaption.weight(.medium)).foregroundColor(Color.appOnSurface.opacity(0.8))
                        Spacer()
                    }
                    .padding(8)
            ...
            if available.isEmpty && gym.crowdsource == nil {
                Text("Aucune donnée communauté pour ce gym — sélectionne un profil ci-dessus.")
                    .font(.appCaption).foregroundColor(Color.appOnSurface.opacity(0.35)).italic()
            }
        }
        .padding(14).background(Color.appCard).cornerRadius(14)
    }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            Image(systemName: "arrow.right")
                                .font(.appCaption).foregroundColor(Color.appOnSurface.opacity(0.3))
                            Text(sub.substitute)
                        ...
                        HStack(spacing: 4) {
                            Image(systemName: "dumbbell.fill")
                                .font(.appMicro).foregroundColor(Color.appOnSurface.opacity(0.3))
                            Text(sub.equipment)
                                .font(.appCaption).foregroundColor(Color.appOnSurface.opacity(0.4))
                        }
