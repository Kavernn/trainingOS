...
struct PerformanceConditionsCard: View {
    ...
    private var scoreColor: Color {
        guard let s = data.alignmentScore else { return Color.appOnSurface.opacity(0.35) }
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: condition.matchToday ? "checkmark.circle.fill" : "circle")
                .font(.appLabel)
                .foregroundColor(condition.matchToday ? .forge : Color.appOnSurface.opacity(0.22))
            ...
            Text(condition.label)
                .font(.appCaption)
                .foregroundColor(condition.matchToday ? Color.appOnSurface.opacity(0.80) : Color.appOnSurface.opacity(0.42))
                .lineLimit(1)
...
private struct AlignmentScoreBanner: View {
    ...
    private var scoreColor: Color {
        guard let s = data.alignmentScore else { return Color.appOnSurface.opacity(0.40) }
        if s >= 80 { return .forge }
        if s >= 50 { return Color.trendNeutral }
        return Color.trendNegative
    }
...
private struct ConditionDetailRow: View {
    ...
    private var matchColor: Color { condition.matchToday ? .forge : Color.appOnSurface.opacity(0.28) }
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(condition.label)
                    ...
                    .foregroundColor(condition.matchToday ? Color.appOnSurface.opacity(0.88) : Color.appOnSurface.opacity(0.50))
