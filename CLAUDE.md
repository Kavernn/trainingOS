...
- **Anti-fantôme backend** : un endpoint ne retourne JAMAIS une valeur plausible inventée quand les données sources sont absentes ou en erreur. L'absence est un état explicite du payload (champ `null` + `reason`, ou `insufficient_data: true`). Troisième occurrence corrigée juillet 2026 : `readiness {score:65}` (readiness.py:846-855), `mesocycle_status` calendaire (analytics_load.py:353-354), `overtraining_risk` low fantôme (analytics_load.py:260-270). Sur exception : `logger.warning` + retour explicite, jamais `pass` silencieux qui retombe sur un calcul inventé.
...
- **Hiérarchie d'exploration (RÈGLE)** : Pour toute question de STRUCTURE — qui appelle quoi, dépendances, définitions, navigation, "où vit X", "qui consomme Y" — consulter le graphe EN PREMIER (`graphify query` pour une question ciblée, `GRAPH_REPORT.md` pour la vue large). NE PAS lancer grep/glob pour ces questions structurelles. Grep est réservé au CONTENU TEXTUEL que le graphe n'indexe pas : strings littérales, valeurs hardcodées (ex : 1.0324), commentaires, SQL, texte dans les données. Si une exploration structurelle commence par grep alors que le graphe pouvait répondre, c'est une erreur de méthode. **Exception Python/api/ ci-dessous.**
- **Python `api/` — graphify ne résout PAS les imports sys.path (RÈGLE)** : les modules `api/*.py` s'importent en style plat (`from db_exercises import get_exercise_id`, pas `from api.db_exercises import ...`) car Vercel exécute avec `api/` sur `sys.path`. Le parseur graphify scanne depuis la racine du repo et **ne crée aucun edge `calls` cross-fichier pour ces imports**. Preuve : `graphify affected "get_exercise_id"` retourne 1 caller (intra-fichier) ; grep retourne 11 sites sur 3 fichiers (`db_sessions.py`, `db_exercises.py`, `routes/workout_exercises.py`). Donc pour toute question **"qui appelle / qui consomme / dépendances / affected" portant sur du Python dans `api/`** : **grep est la source de vérité, graphify ne compte pas comme preuve**. graphify reste OK pour : définitions et structure intra-fichier. Swift : non vérifié — le même défaut de resolver pourrait exister, à tester avant de s'y fier pour les questions cross-fichier. Ne PAS refactorer les imports vers `api.` pour plaire à l'outil (risque prod).
- **Warning graphify « -N nodes » (RÈGLE)** : ne PAS lancer `--force` sans avoir compté le delta ligne par ligne. Méthode : `python3 -c "import json; g=json.load(open('graphify-out/graph.json')); print([n['label'] for n in g['nodes'] if n.get('source_file')=='<file>'])"` pour lister les nœuds par fichier touché du commit, comparer aux symboles réellement supprimés/renommés dans les edits. `--force` UNIQUEMENT si delta expliqué. Cause fréquente : edits successifs entre deux updates — le warning ne trace pas les mtimes intermédiaires.
- **« 2 fewer nodes » post-fix nutrition (TODO — non résolu)** : incident 2026-09-03 après ajout d'une ligne d'export dans `api/db.py` — `graphify update` a refusé d'écraser en rapportant 2 nodes en moins alors qu'on ajoutait du code. Non reproductible aujourd'hui (état non préservé), cause non prouvée, PAS marqué résolu. À la prochaine récidive, capturer immédiatement AVANT tout `--force` : (a) stderr exact du refus, (b) `git diff` du fichier édité entre les deux runs, (c) `manifest.json` avant/après. Sans ces 3 pièces l'incident restera indiagnosticable.
- **Prévention** : lancer `graphify update` UNIQUEMENT au moment du commit, jamais en milieu de session — un update intermédiaire crée un snapshot que les edits suivants invalident, source du faux warning. La règle de lecture ci-dessus soigne le symptôme, celle-ci évite la cause.
