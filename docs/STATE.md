...
- **Historique — volume par groupe musculaire** : `/api/stats_data` produit encore les blocs legacy `muscle_stats` + `muscle_landmarks`. Les anciennes vues Swift `MuscleVolumeView`, `MuscleBreakdownView` et `VolumeLandmarksCard` les consommaient, mais aucun consumer Swift actif de ces blocs n'est désormais identifié. Ils restent temporairement conservés pour compatibilité avec d'éventuels consumers externes inconnus ; le cockpit est la source actuelle des Stats musculaires modernes.
